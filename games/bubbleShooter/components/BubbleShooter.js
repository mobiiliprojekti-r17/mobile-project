...
const { width, height } = Dimensions.get('window');
const BALL_RADIUS = 20;
// Define a level at which the game is over (adjust as needed)
const GAME_OVER_LEVEL = height - 150;  // For example, balls below this y-position end the game
...
const BubbleShooter = ({ navigation }) => {
  ...
  const [time, setTime] = useState(0);
  const timerRef = useRef(null);
  // Use a ref flag to prevent game over logic from running multiple times
  ...
  useEffect(() => {
    ...
    Matter.Events.on(engine, 'collisionStart', (event) => {
      for (let { bodyA, bodyB } of event.pairs) {
        ...
        if ((bodyA === shooter && bodyB === ceiling) || (bodyB === shooter && bodyA === ceiling)) {
          ...
          Matter.Body.setPosition(shooter, {
            x: shooter.position.x,
            y: 60 + BALL_RADIUS + 1 // Position it slightly below the ceiling
          });
          setStaticBalls(prev => [...prev, shooter]);
    ...
    const update = () => {
      ...
      // --- New section: Check if any static ball is below the game-over level ---
      if (!gameOverTriggered.current) {
        // Loop through static balls to see if any crosses the defined threshold
        for (let ball of staticBallsRef.current) {
          if (ball.position.y >= GAME_OVER_LEVEL) {
  ...
  // Existing useEffect for checking win condition (cleared board)
  useEffect(() => {
